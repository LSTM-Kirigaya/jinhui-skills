...
tags:
  ...
  - 网站
model: deepseek-chat
rootUrl: https://raw.githubusercontent.com/LSTM-Kirigaya/jinhui-skills/main/skills/jinhui-stack-debug/SKILL.md
---
# Jinhui Stack Debug
...
| 类型 | 路径 | 描述 |
|-----|------|-----|
| 前端开发准则 | [./frontend-coding-standards/SKILL.md](./frontend-coding-standards/SKILL.md) | 前端组件开发规范。适用于 Vue、React、Next.js、Taro 等框架。组件函数与样式分离，文件行数控制（1000 行阈值），模块拆分与依赖迁移，提升复用性和健壮性。 |
| 测试集构建 | [./build-test-suite/SKILL.md](./build-test-suite/SKILL.md) | 软件测试集构建指南。项目初始化时检查测试集，遵循五大准则。 |
| 迭代习惯 | [./good-iteration-habits/SKILL.md](./good-iteration-habits/SKILL.md) | 良好的软件迭代习惯。测试优先、复用现有接口、功能影响评估、重构验证。 |
| 重构验证 | [./refactoring-with-verification/SKILL.md](./refactoring-with-verification/SKILL.md) | 重构 UI 时的功能完整性验证指南。截图记录、功能清单、前后对比，确保不丢失任何功能。 |
## Usage Examples
