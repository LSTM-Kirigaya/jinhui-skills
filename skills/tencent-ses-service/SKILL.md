...
HTML 模板中的链接**必须包含完整域名**，仅允许对路径或参数部分使用模板变量。**禁止出现将完整 URL（含协议和域名）整体替换为变量的情况**。
...
- [ ] 邮件内容中的链接使用 HTTPS
- [ ] 邮件模板名称清晰明确，能够直接判断邮件用途（如「注册验证码邮件」合法，「邮件发送」非法）
- [ ] 邮件模板中的链接包含完整域名，未将协议+域名部分整体替换为变量（如 `{{url}}` 非法，`https://example.com/path?id={{id}}` 合法）
